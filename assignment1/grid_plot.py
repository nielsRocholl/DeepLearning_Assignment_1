...
def loss_grid_plot(args, epochs=10, max_loss = 1.0, save_path=None):
    ...
    fig, axes = plt.subplots(nrows=rows, ncols=cols, sharex=True, sharey=True, figsize=(8,10), squeeze=False)
    for col in axes:
        for ax in col:
            #ax.set_yscale("log")
    ...
    for (n_m, model_key) in enumerate(args.model):
        for (n_a, aug_key) in enumerate(args.augment):
            ...
            ax.set_ylabel("{}\n{}".format(model_names[model_key], aug_names[aug_key]), size=15)
    ...
    for (n_a, act_key) in enumerate(args.activation):
        for (n_o, opt_key) in enumerate(args.optimizer):
            ...
            ax.set_title("{}\n{}".format(act_names[act_key], opt_names[opt_key]), size=15)
    plt.subplots_adjust(left=0.0, bottom=None, right=0.9, top=None, wspace=0.05, hspace=0.05)
                ...
                for (n_o, opt_key) in enumerate(args.optimizer):
                    ...
                    for rep in run_args.repeats:
                        try:
                            ...
                            axes[row][col].plot(history['loss'])
                            axes[row][col].plot(history['val_loss'], ls=':')
                        except:
                            pass
